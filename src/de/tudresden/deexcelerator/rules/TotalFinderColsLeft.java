				...
				for (int y = sd.rowStart; y<sd.rowEnd; y++) {
					if (!sd.emptyRows.contains(y)) {
						...
						if (findSum == null || findSum == 0)
							continue;
						double sum = 0;
						//Abweichung neu setzen
						if (findSum > 50000)
							abweichung = 5;
						else
							abweichung = gd.getCp().deviationCol;
						for (int x = totalSpalte-1; x>=sd.colStart; x--) {
