...
import de.tudresden.deexcelerator.util.OutputCell;
...
public class OutputObjectCell {
     ...
     */
	public OutputObjectCell (DocumentInformation gd, AnalyzeInformation sd) {
		this.action(gd, sd);
	}
	...
	private void action(DocumentInformation gd, AnalyzeInformation sd){
		...
		matrix = new OutputCell[zaehlerSpalten][zaehlerZeilen];
		headerInfo = new Map[zaehlerSpalten];
		...
		for (int x=sd.colStart; x<sd.colEnd; x++)
		{
			if (!sd.emptyCols.contains(x)) {
				...
				map.put("title", attribut);
				headerInfo[spalte] = map;
				spalte++;
			}
		}
		zaehlerZeilen = 0;
		...
		for (int x=sd.colStart; x<sd.colEnd; x++)
		{
			if (!sd.emptyCols.contains(x)) {
				for (int y=sd.headerFinish; y<sd.rowEnd; y++)
				{
					if (!sd.emptyRows.contains(y))
					{
						OutputCell oc = new OutputCell();
						Map<String,String> map = headerInfo[zaehlerSpalten];
						...
						{
							oc.date = gd.getFc().getDateValue(x, y);
						}
						else if (map.get("type").equals("integer"))
						{
							oc.integerNumber = gd.getFc().getIntegerValue(x, y);
						}
						else if (map.get("type").equals("double"))
						{
							oc.doubleNumber = gd.getFc().getDoubleValue(x, y);
						}
						matrix[zaehlerSpalten][zaehlerZeilen] = oc;
						zaehlerZeilen++;
					}
				}
				zaehlerZeilen=0;
	 ...
	 */
	public static OutputObjectCell fromJson(String json) {
        return gson.fromJson(json, OutputObjectCell.class);
    }
